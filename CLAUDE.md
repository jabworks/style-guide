...
- `configs/` — TypeScript config files (`base`, `typescript`, `react`, `react-native`, `expo`, `next`, `node`, `library`, `vitest`), plus the `imperative-render` override and the opt-in JS-plugin layers `expo-plugin` and `react-native-plugin`
