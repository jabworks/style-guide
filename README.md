...
- **`@jabworks/oxlint-config`**: Shareable oxlint presets (base, typescript, react, reactNative, expo, next, node, library, vitest) ported from `@jabworks/eslint-plugin`. Requires `oxlint` ≥1.85.0 and `oxlint-tsgolint` for type-aware rules.
